            ...
            if(current_char != '.')
            {
                for(int row_i_compare = 0; row_i_compare < antenna_map.size(); row_i_compare++)
                {
                    for(int col_i_compare = 0; col_i_compare < antenna_map[row_i].size(); col_i_compare++)
                    {
                        ...
                        {
                            int anti_row_id = row_i + (row_i - row_i_compare);
                            int anti_col_id = col_i + (col_i - col_i_compare);

                            cout << "HIT: " << row_i << "," << col_i << " and " <<
                            row_i_compare << "," << col_i_compare << " gives " <<
                            anti_row_id << "," << anti_col_id << endl;
                            
                            if(anti_row_id >= 0 && anti_row_id < map_nrows &&
