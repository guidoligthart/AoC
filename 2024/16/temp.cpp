...
vector<vector<int>> get_neighbours(int x, int y, maze_map &map, bool visited_matters = true){
    ...
    for(auto direction : dir){
        ...
        if(dest_y >= 0 && dest_y < map.character.size() && dest_x >= 0 && dest_x < map.character.size()){
...
void place_chairs(int x, int y, maze_map &m_map){
    ...
    for(auto neighbour:neighbours){
        ...
        if(x == 5 & y == 7){
            cout <<neighbour[0] << "," << neighbour[1] << ": " << current_cost << " - " << neighbour_cost << " = " << dif << endl;
        }
        if (dif == 1 || dif == 1001){
            place_chairs(neighbour[1], neighbour[0], m_map);
        }
    }
}
