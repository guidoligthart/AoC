...
bool tolerable_report(vector<int> levels){
    int vectorSize = levels.size();
    vector<int> subset_levels;
    ...
    for (int i = 0; i < vectorSize; i++)
    {
        ...
        {
            if(i!=j){
                subset_levels.push_back(levels[j]);
            }
        }

        if(safe_report(subset_levels)){
            return true;
        }
        subset_levels.clear();
    }
...
int main() {
    ...
    while (std::getline(inFile, entry)) {
        ...
        safe_counter += tolerable_report(numbers);
    }
